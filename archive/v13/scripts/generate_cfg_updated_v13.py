...
def canonical_symbol(name):
    """Normalize the leading underscore used for C symbols on macOS."""
    name = name.strip()
    return name[1:] if name.startswith("_") else name


def select_function(parsed_functions, preferred="func0", *, allow_largest_fallback=False):
    """Select one function from parse_assembly() output."""
    exact = [item for item in parsed_functions if canonical_symbol(item[0]) == preferred]
    if len(exact) == 1:
        return exact[0]
    if len(exact) > 1:
...
def generate_cfg_text(source, target_info, input_format="auto", function_name=None,
    ...
    only that function (macOS leading underscore is normalized). For Mach-O
    ...
    function.
    """
    parsed_functions = parse_assembly(source, target_info, input_format=input_format)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arch", choices=["x86", "arm", "riscv"], required=True)
